...
func TestRefererParser(t *testing.T) {
	parser, err := New("testdata/test.yaml")
	assert.NoError(t, err)

	currentUrl := "https://www.snowplowanalytics.com/"

	for _, tdata := range testData {

		res, _ := parser.Parse(tdata.uri, currentUrl)

		if tdata.known != res.Known {
			t.Errorf("Bad known for %+v, expected:%+v, got:%v", tdata, tdata.known, res.Known)
		}
		if tdata.source != res.Source {
			t.Errorf("Bad source for %+v, expected:%+v, got:%v", tdata, tdata.source, res.Source)
		}
		if tdata.medium != res.Medium {
			t.Errorf("Bad medium for %+v, expected:%+v, got:%v", tdata, tdata.medium, res.Medium)
		}
		if tdata.term != res.Term {
			t.Errorf("Bad term for %+v, expected:%+v, got:%v", tdata, tdata.term, res.Term)
		}
	}
}